...
export class StorytellerAgent {
    ...
    async continueStory(userPrompt) {
            const messages = [
                ...
                { role: "user", content: `Setze das folgende Märchen fort. Der bisherige Teil ist: "${this.story.join(' ')}". Der Zuhörer möchte das du: "${userPrompt}". Generiere drei bis fünf Sätze, und baue eine unerwartete Wendung in die Handlung ein. `}
        ];
